mutable struct EnergyStorage
   ...
   cd::Float64
   cc::Float64
   function EnergyStorage(index, bus, Pdmax, Pcmax, Emax, Emin, Hemax, cd, cc)
      e = new()
      e.index  = index
      e.bus = bus
      e.Pdmax = Pdmax
...
function load_ES(datadir)
  # READ RAW DATA
  println(">>>>> Reading feeder data from $(datadir)")

  ES_raw = CSV.read("$datadir/ES.csv", DataFrame)
  #ES_raw = DataFrame(CSV.File("$datadir/ES.csv"))

  ESs = []
  for e in 1:nrow(ES_raw)
      index = ES_raw[e, :index]
      bus = ES_raw[e, :node]
      Pdmax = ES_raw[e, :pdmax]
      Pcmax = ES_raw[e, :pcmax]
      Emax = ES_raw[e, :Emax]
      Emin = ES_raw[e, :Emin]
      Hemax = ES_raw[e, :Hemax]
      cd = ES_raw[e, :cd]
      cc = ES_raw[e, :cc]
      newe = EnergyStorage(index, bus, Pdmax, Pcmax, Emax, Emin, Hemax, cd, cc)
      push!(ESs, newe)
  end
  return ESs
end
...
function load_timeseries(datadir)
  ...
  wind_data = Matrix(CSV.read("$datadir/wind_data.csv", DataFrame, header=false))
  load_data = Matrix(CSV.read("$datadir/load_data.csv", DataFrame, header=false))
  Hw = Matrix(CSV.read("$datadir/Hw.csv", DataFrame, header=false))
  return wind_data, load_data, Hw
end
